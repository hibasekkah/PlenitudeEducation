...
function getPresenceBadge(presenceStatus, status) {
  // Ne pas afficher le badge de présence pour les séances à venir
  ...
  // Pour les séances en cours ou terminées sans données de présence
    ...
    <div className="p-6">
      ...
      <div className="space-y-3">
        {seances.map((seance, index) => {
          ...
          const presenceStatus = seance.presence_participant;
          ...
          return (
            ...
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-4 flex flex-col gap-2">
                    ...
                    {getPresenceBadge(presenceStatus, status)}
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-3">
                ...
                {/* Section présence détaillée pour les séances passées */}
                {status === 'completed' && (
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                      <UserCheck className="h-4 w-4 text-gray-600" />
                      <span className="text-sm font-medium text-gray-900">Présence</span>
                    </div>
                    <div className="text-sm">
                      {presenceStatus === true ? (
                        <div className="text-green-700">
                          ✓ Vous étiez présent(e) à cette séance
                        </div>
                      ) : presenceStatus === false ? (
                        <div className="text-red-700">
                          ✗ Vous étiez absent(e) à cette séance
                        </div>
                      ) : (
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="border-t pt-3">
          <h4 className="text-sm font-medium text-gray-900 mb-2">Présence</h4>
          <div className="grid grid-cols-3 gap-4 text-xs">
            <div className="text-center">
              <div className="font-semibold text-green-600">
                {seances.filter(s => s.presence_participant === true).length}
              </div>
              <div className="text-gray-600">Présent</div>
            </div>
            <div className="text-center">
              <div className="font-semibold text-red-600">
                {seances.filter(s => s.presence_participant === false).length}
              </div>
            ...
            <div className="text-center">
              <div className="font-semibold text-yellow-600">
                {seances.filter(s => 
                  getSeanceStatus(s.date, s.heure_debut, s.heure_fin) === 'completed' && 
                  s.presence_participant === null
                ).length}
              </div>
              <div className="text-gray-600">Non renseigné</div>
            </div>
          </div>
        </div>
      </div>
